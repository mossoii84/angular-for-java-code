...
@Component({
  ...
  imports: [CommonModule, ButtonModule, TableModule, DialogModule, ReactiveFormsModule, FormsModule, DatePickerModule, MessageModule, 
...
export class HomePageComponent implements OnInit {
  ...
  ngOnInit(): void {
      this.serviceTasksList.getTasks().subscribe (
        (tasks: Task[])=> 
        this.store.dispatch(TasksListActions.retrievedTaskList({ tasks }))
      )

      this.tasks$ = this.store.select(selectTaskList).pipe(
        map((tasks) => [...tasks])
      );
